...
- **优化器**: AdamW，学习率 1e-3（只训练分割头，可以用较大学习率）
- **损失函数**: CrossEntropyLoss（忽略 label=255 的边界像素）
- **训练轮数**: 50 epochs
- **数据增强**: 随机水平翻转、随机缩放（0.5-2.0）、随机裁剪
...
│   ├── backbone.py          # DINOv3 和 ResNet50 backbone 封装
│   └── segmentor.py         # Backbone + Head 组合
├── datasets/
│   └── voc.py               # Pascal VOC 数据加载与增强
├── train.py                 # 训练脚本
├── evaluate.py              # 评估脚本（计算 mIoU）
├── visualize.py             # 可视化脚本（PCA、分割结果）
└── configs/
    └── default.yaml         # 训练超参数配置（可选）
```
